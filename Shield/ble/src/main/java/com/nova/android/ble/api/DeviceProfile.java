...
public class DeviceProfile {
    ...
    private static final String TAG = "[Nova][Ble][DeviceProfile]" ;
    ...
    private void checkDevice(Context context) {
        ...
        if (BleUtils.getBluetoothAdapter(context).getBluetoothLeAdvertiser() != null) {
            this.rating++;
        ...
        if (BleUtils.getBluetoothAdapter(context).isOffloadedFilteringSupported()) {
            this.rating++;
        ...
        if (BleUtils.getBluetoothAdapter(context).isOffloadedScanBatchingSupported()) {
            this.rating++;
    ...
    public static boolean isLeExtendedRangeSupported(Context context) {
        return Build.VERSION.SDK_INT >= 26 && BleUtils.getBluetoothAdapter(context).isLeCodedPhySupported();
    }

    public static boolean isLeDoubleRateSupported(Context context) {
        return Build.VERSION.SDK_INT >= 26 && BleUtils.getBluetoothAdapter(context).isLe2MPhySupported();
    }

    public static boolean isLeExtendedAdvertisingSupported(Context context) {
        return Build.VERSION.SDK_INT >= 26 && BleUtils.getBluetoothAdapter(context).isLeExtendedAdvertisingSupported();
    }

    private static boolean isLePeriodicAdvertisingSupported(Context context) {
        return Build.VERSION.SDK_INT >= 26 && BleUtils.getBluetoothAdapter(context).isLePeriodicAdvertisingSupported();
    }
    ...
    public boolean isAdvertisingSupported(Context context) {
        return (BleUtils.getBluetoothAdapter(context) == null || BleUtils.getBluetoothAdapter(context).getBluetoothLeAdvertiser() == null) ? false : true;
    }
